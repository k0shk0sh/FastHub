 ...
 */
class FeedsCell(private val node: FeedModel) : SimpleCell<FeedModel>(node), Updatable<FeedModel> {

    override fun getLayoutRes(): Int = R.layout.feeds_main_screen_row_item

    override fun areContentsTheSame(newItem: FeedModel): Boolean = node == newItem

    override fun getChangePayload(newItem: FeedModel): Any = newItem

    override fun onBindViewHolder(holder: SimpleViewHolder, position: Int, context: Context, payload: Any?) {
        holder.itemView.apply {
            val item = payload as? FeedModel ?: item
