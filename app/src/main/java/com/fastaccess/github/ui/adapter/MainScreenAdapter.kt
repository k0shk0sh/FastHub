 ...
 */
class MainScreenAdapter : ListAdapter<MainScreenModel, RecyclerView.ViewHolder>(DIFF_CALLBACK) {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            MainScreenModelRowType.FEED.rowType -> ProfileFeedsViewHolderr(parent)
            MainScreenModelRowType.NOTIFICATION.rowType -> NotificationsViewHolder(parent)
            MainScreenModelRowType.ISSUES.rowType -> MainIssuesPrsViewHolder(parent)
            MainScreenModelRowType.PRS.rowType -> MainIssuesPrsViewHolder(parent)
            else -> TitleSectionViewHolder(parent)
        }
    }
